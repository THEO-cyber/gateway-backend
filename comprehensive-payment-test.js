...
async function testServerHealth() {
  ...
  try {
    ...
    if (response.status === 200 && response.data.status === 'OK') {
      logTest(testName, true, `Server healthy, response time: ${duration.toFixed(2)}ms`);
      return true;
    } else {
...
async function testPaymentFee() {
  ...
  try {
    ...
    if (response.status === 200 && 
      ...
      logTest(testName, true, `Fee correctly set to ${response.data.data.formattedAmount}`);
      return response.data;
    } else {
      logTest(testName, false, 'Fee endpoint returned unexpected data', response.data);
      return null;
    }
  } catch (error) {
...
async function testUserAuth() {
  ...
  try {
    ...
    logTest(testName, false, `Authentication failed: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function testPaymentInitiation() {
  ...
  try {
    ...
    const response = await axios.post(`${API_BASE_URL}/payment/initiate`, paymentData, { headers });
    ...
    if (response.status === 200 && 
      ...
      logTest(testName, true, `Payment initiated: ${response.data.data.transactionId}`);
    ...
    logTest(testName, false, `Payment initiation error: ${error.response?.data?.message || error.message}`);
    return null;
  }
}
...
async function testDuplicatePaymentPrevention() {
  ...
  try {
    ...
    try {
      await axios.post(`${API_BASE_URL}/payment/initiate`, paymentData, { headers });
      logTest(testName, false, 'System allowed duplicate payment - atomicity violation!');
      return false;
      ...
      if (duplicateError.response?.status === 400 && 
        ...
        logTest(testName, false, 'Unexpected error during duplicate test', duplicateError.response?.data);
        return false;
      }
    }
  } catch (error) {
    logTest(testName, false, `Duplicate prevention test failed: ${error.message}`);
    return false;
  }
}
...
async function testPaymentStatus(transactionId) {
  ...
  try {
    ...
    const headers = { 'Authorization': `Bearer ${authToken}` };
    const response = await axios.get(`${API_BASE_URL}/payment/status/${transactionId}`, { headers });
    ...
    logTest(testName, false, `Status check error: ${error.response?.data?.message || error.message}`);
    return null;
  }
}
...
async function testWebhookSecurity() {
  ...
  try {
    ...
    const correctSignature = generateWebhookSignature(webhookPayload, TEST_CONFIG.webhookSecret);
    
    try {
      const response = await axios.post(`${API_BASE_URL}/payment/webhook`, webhookPayload, {
        headers: { 'X-Signature': correctSignature }
      });
      
      if (response.status === 200) {
        logTest(testName, true, 'Webhook signature verification working (returned 200 even for unknown transaction)');
      } else {
        logTest(testName, false, 'Webhook failed with correct signature', response.data);
      }
    } catch (error) {
      ...
      if (error.response?.status === 200) {
        ...
        logTest(testName, false, `Webhook security test failed: ${error.message}`);
      }
    }
...
async function testPaymentHistory() {
  ...
  try {
    ...
    const headers = { 'Authorization': `Bearer ${authToken}` };
    const response = await axios.get(`${API_BASE_URL}/payment/history?page=1&limit=5`, { headers });
    
    if (response.status === 200 && response.data.success === true) {
      logTest(testName, true, `Retrieved ${response.data.data.payments.length} payments`);
      return response.data.data;
    } else {
    ...
    logTest(testName, false, `Payment history error: ${error.response?.data?.message || error.message}`);
    return null;
  }
}
...
async function testPhoneNumberValidation() {
  ...
  try {
    ...
    try {
      await axios.post(`${API_BASE_URL}/payment/initiate`, invalidPaymentData, { headers });
      ...
      if (error.response?.status === 500 && 
        ...
        logTest(testName, false, 'Unexpected error during phone validation', error.response?.data);
        return false;
      }
    }
  } catch (error) {
...
async function testDatabaseAtomicity() {
  ...
  try {
    ...
    const historyBefore = await axios.get(`${API_BASE_URL}/payment/history`, { headers });
    const countBefore = historyBefore.data.data.pagination.total;
    
    // Wait a moment, then get count again to ensure consistency
    await sleep(100);
    const historyAfter = await axios.get(`${API_BASE_URL}/payment/history`, { headers });
    const countAfter = historyAfter.data.data.pagination.total;
    
    if (countBefore === countAfter) {
      logTest(testName, true, 'Database consistency maintained between requests');
      return true;
    } else {
      logTest(testName, false, `Database inconsistency detected: ${countBefore} vs ${countAfter}`);
      return false;
    }
  ...
  try {
    const tests = [
      // Test 1: No auth token
      {
        ...
        request: () => axios.post(`${API_BASE_URL}/payment/initiate`, { phone: '671234567' }),
        expectedStatus: 401
      },
      // Test 2: Missing phone number
      {
        name: 'Missing Phone Number',
        request: () => axios.post(`${API_BASE_URL}/payment/initiate`, 
          { purpose: 'registration_fee' }, 
      ...
      // Test 3: Invalid transaction ID
      {
        ...
        request: () => axios.get(`${API_BASE_URL}/payment/status/INVALID_ID`, 
          { headers: { 'Authorization': `Bearer ${authToken}` } }),
        expectedStatus: 404
      }
    ];
    ...
    for (const test of tests) {
      try {
        ...
        if (error.response?.status === test.expectedStatus) {
          ...
          console.log(`❌ ${test.name}: Expected ${test.expectedStatus}, got ${error.response?.status}`);
        }
      }
    }
    
    const allPassed = passedTests === tests.length;
    logTest(testName, allPassed, `${passedTests}/${tests.length} error handling tests passed`);
    return allPassed;
...
async function checkDeploymentReadiness() {
  ...
  const checks = {
    'Environment Variables': process.env.NKWAPAY_API_KEY && process.env.MONGODB_URI,
    'Database Connection': testResults.tests.find(t => t.name === 'Server Health Check')?.passed,
    'Payment Integration': testResults.tests.find(t => t.name === 'Payment Fee Endpoint')?.passed,
    'Authentication': testResults.tests.find(t => t.name === 'User Authentication')?.passed,
    'Atomicity': testResults.tests.find(t => t.name === 'Payment Initiation (Atomicity)')?.passed,
    'Security': testResults.tests.find(t => t.name === 'Webhook Security (Signature Verification)')?.passed,
    'Error Handling': testResults.tests.find(t => t.name === 'Error Handling')?.passed,
    'Data Validation': testResults.tests.find(t => t.name === 'Phone Number Validation')?.passed
  };
...
async function runAllTests() {
  ...
  if (testResults.performance.healthCheck) {
    console.log(`Health Check: ${testResults.performance.healthCheck.toFixed(2)}ms`);
  }
