...
async function testPaymentFlow() {
  try {
    ...
    try {
      ...
      console.log(
        "⚠️  Login failed, you may need to register first or check credentials",
      );
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(
            "   Total payments:",
            historyResponse.data.data.pagination.total,
          );
        } catch (error) {
          console.log(
            "⚠️  History fetch failed:",
            error.response?.data?.message || error.message,
          );
        }
      }, 3000);
...
async function testAdminEndpoints() {
  ...
  try {
    ...
    console.log(
      "   Total revenue:",
      statsResponse.data.data.totalRevenue,
      "FCFA",
    );
  } catch (error) {
