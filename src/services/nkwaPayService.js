...
function formatPhoneNumber(phone) {
  ...
  throw new Error('Invalid phone number format. Expected Cameroon number starting with 6XXXXXXXX or 237XXXXXXXXX');
}

async function initiateNkwaPayment({ phoneNumber, userId = null, userEmail = null, purpose = 'registration_fee', description = 'HND Gateway Registration Fee' }) {
  if (!NKWAPAY_API_KEY) {
  ...
  try {
    ...
    console.log(`[NkwaPayService] Payment initiated successfully: ${transactionId}`);
    ...
    try {
      const failedPayment = await Payment.findOne({ transactionId }).sort({ createdAt: -1 });
      if (failedPayment && failedPayment.status === 'pending') {
        await failedPayment.markAsFailed(err.message, err.response?.status?.toString());
      }
    } catch (updateError) {
      console.error("[NkwaPayService] Error updating failed payment:", updateError.message);
    }
    ...
    if (err.response) {
      ...
      throw new Error(`Payment failed: ${err.response.data?.message || 'Unknown error from payment provider'}`);
...
async function checkPaymentStatus(transactionId) {
  try {
    ...
    console.error(`[NkwaPayService] Error checking payment status: ${err.message}`);
    if (err.response) {
...
async function processWebhook(payload, signature) {
  try {
    ...
    if (!payment) {
      console.warn(`[NkwaPayService] Webhook received for unknown payment: ${reference}`);
    ...
    if (status === 'successful' || status === 'completed') {
      ...
      payment.status = 'processing';
      await payment.save();
      console.log(`[NkwaPayService] Payment status updated: ${reference} - ${status}`);
    }
